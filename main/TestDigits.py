...
# generate a string that contains the status of each nixie
def PrintNixieStatus() :
    global blankdigits
    output = ""
    for index,x in enumerate(blankdigits):
...
# we will stop this time first so MENU starts first 
#
...
while True:
   ...
   print(" the NIXIEs currently active:")
   print(PrintNixieStatus()) 
   ...
   if raw_option == "t":
      ...
      while True:
         ...
         if raw_option2.isdigit():
            ...
            print("The %s digit is being driven now" %(ind-1))
            print("the timer is set for %s minutes" % (burntime/60))
      ...
      while True:
         ...
         if raw_option2.isdigit():
            ...
            burntime = int(raw_option2)
            burntime = burntime*60
            print("The %s digit is being driven now" %(ind-2))
            print("the timer is set for %s minutes" % (burntime/60))
      ...
      while True:
         ...
         if raw_option2.isdigit():
            # we will start time by writting to first odd number and cycle zero, timer will update from 2,4,6,8
            if ind%2 == 0:
               int += 1
            DigitSec.Write_Display([ind,ind,ind,ind,ind,ind],blankdigits)
            ind += 2
